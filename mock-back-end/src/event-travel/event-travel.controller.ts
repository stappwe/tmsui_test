...
@Controller('Twinning/api/TMS/event/travel')
export class EventTravelController {
  @Get('travel-list/:eventId')
  getExpertRegistrationList(@Headers('authorization') authorization: string,
                            @Param('eventId') eventId: number,
                            @Res() res: Response): void {
    console.log('Twinning/api/TMS/event/travel/travel-list - eventId: ' + eventId);

    createReadStream(path.join(__dirname, '../resources/json/events/event/travel/travelList.json')).pipe(res);
  }
  @Get('participant-list/:eventId')
  getEventTravelParticipantList(@Headers('authorization') authorization: string,
                            @Param('eventId') eventId: number,
                            @Res() res: Response): void {
    console.log('Twinning/api/TMS/event/travel/participant-list - eventId: ' + eventId);

    createReadStream(path.join(__dirname, '../resources/json/events/event/travel/travelParticipantList.json')).pipe(res);
  }
